		...
		<div class="panel-body">
			<div class="row">
				<div class="col-sm-6 pad"><strong class="text-primary"><?php echo __('Name'); ?></strong>
					<strong class="text-success"><?php echo h($post['Exam']['name']); ?></strong></div>
				<div class="col-sm-6 pad"><strong class="text-primary"><?php echo __('Type'); ?></strong>
					<strong class="text-success"><?php echo __($post['Exam']['type']); ?></strong></div>
			</div>
			<div class="row">
				<div class="col-sm-6 pad"><strong class="text-primary"><?php echo __('Passing Percentage'); ?></strong>
					<strong class="text-success"><?php echo $post['Exam']['passing_percent']; ?>%</strong></div>
			...
			<div class="row">
				<div class="col-sm-6 pad"><strong class="text-primary"><?php echo __('Negative Marking'); ?></strong>
					<strong class="text-success"><?php echo __($post['Exam']['negative_marking']); ?></strong></div>
				<div class="col-sm-6 pad"><strong
						class="text-primary"><?php if ($totalMarks > 0) echo __("Total Marks"); ?></strong>
					<strong class="text-success"><?php if ($totalMarks > 0) echo $totalMarks; ?></strong></div>
				<table class="table">
					<tr>
					...
					foreach ($subjectDetail as $k => $sd):
						...
						<tr>
							...
							<?php if ($post['Exam']['exam_mode'] == "T") { ?>
								<td><strong
									class="text-success"><?php echo $sd['duration']; ?>
					...
					<tr>
						...
						<?php if ($post['Exam']['exam_mode'] == "T") { ?>
							<td><strong
								class="text-danger"><?php echo $totalDuration; ?>
								&nbsp;<?php echo __('Mins'); ?></strong>
							</td><?php } ?>
					</tr>
				</table>
				<?php if (strlen($post['Exam']['syllabus']) > 0) { ?>
					...
					<div
						style="height: 190px;overflow: auto; overflow-y: scroll;padding: 10px;"><?php echo $post['Exam']['syllabus']; ?></div>
				<?php } ?>
				<?php if ($showType == 'free' || $showType == 'today') { ?>
					<div><?php echo $this->Form->button('<span class="fa fa-forward"></span> ' . __('Attempt Now'), array('onclick' => "javascript:showpop_up('" . $this->Html->url(array('controller' => 'Exams', 'action' => 'guidelines', $id)) . "')", 'data-toggle' => 'tooltip', 'title' => __('Attempt Now'), 'escape' => false, 'class' => 'btn btn-success')); ?></div>
				<?php } ?>
			</div>
		</div>
	</div>
