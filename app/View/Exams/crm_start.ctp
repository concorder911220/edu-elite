...
<?php if ($post['Exam']['pause_exam'] == "1") { ?>
	<script type="text/javascript">
		$(window).on('unload', function () {
			var URL =
				"<?php echo $this->Html->url(array('controller' => 'Exams', 'action' => 'pause', $post['Exam']['id'])); ?>";
			var data = "";
...
<div class="center_screen" id="exam-loading-start" style="display: block">
	<div style="font-size: 22px;font-weight: bolder;text-align: center; color: #a1a09e;">
		<?php echo __('Loading'); ?>...<span class="progress_percentage">1%</span>
	</div>
	<div class="progress">
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-top: 50px">
		<div class="col-md-9 col-sm-12 col-xs-12">
			...
			<?php if ($post['Exam']['exam_mode'] == "T") { ?>
				<div class="exam-sections">
					...
					foreach ($userSectionSubject as $subjectName => $quesArr):
						$temp++;
						...
						<div class="exam-dpopup"><a id="btnSection<?php echo $quesArr[0]['ExamStat']['tsubject_id']; ?>"
								class="exam-SubjectButton <?php echo $subjectHighlight; ?>" <?php if ($quesArr[0]['ExamStat']['is_section'] == "1") { ?>onclick="SectionButtonClick(<?php echo $quesArr[0]['ExamStat']['tsubject_id']; ?>)" <?php } ?>><?php echo $subjectName; ?></a>
						</div>
					<?php endforeach;
					unset($i);
					unset($value, $temp); ?>
			...
			<?php } else { ?>
				<div class="exam-sections">
					<?php $temp = 0;
					$subjectHighlight = null;
					foreach ($userSectionQuestion as $subjectName => $quesArr):
						$temp++;
						$subjectNameId = str_replace(" ", "", h($subjectName));
						if ($temp == 1) {
							$subjectHighlight = "exam-subjectHighlight";
				...
				<div class="exam-panel <?php echo $subjectSection; ?>" id="quespanel<?php echo $quesNo; ?>">
					...
					<div class="exam-QuestionHeader">
						...
						<span style="float: right;padding-left: 5px;">
							<a class="btn btn-primary btn-xs exam-review"
								onclick="markForReview(<?php echo $quesNo; ?>);"><i class="fa fa-flag"></i></a>
						</span>
						<span style="float: right;padding-left: 5px;">
							<a class="btn btn-primary btn-xs exam-cresponse"
								onclick="resetAnswer(<?php echo $quesNo; ?>);"><i class="fa fa-recycle"></i></a>
						</span>
						...
						<div class="exam-questionBox" id="exam-questionBox" <?php echo $questionStyle; ?>>
							<table class="table">
								<thead>
									<tr>
										<td>
											<div class="">
												<div class="lang-1">
													<?php echo str_replace("<script", "", $userExamQuestion['Question']['question']); ?>
												</div>
												<?php if (is_array($userExamQuestion['Question']['QuestionsLang'])) {
													foreach ($userExamQuestion['Question']['QuestionsLang'] as $k => $item) {
														$k += 2; ?>
														<div class="lang-<?php echo $k; ?>">
															<?php echo str_replace("<script", "", $item['question']); ?>
														</div>
													</div>
												<?php }
												}
										...
										<td>
											<?php if (strlen($userExamQuestion['Question']['answer']) > 1) {
												foreach ($options as $k => $value): ?>
													<div class="checkbox"><label><input name="data[Exam][option_selected][]"
																value="<?php echo $k; ?>" <?php if (in_array($k, explode(",", $userExamQuestion['ExamStat']['option_selected']))) {
																	   echo 'checked=checked';
																   } ?> type="checkbox">
															<?php foreach ($value as $optionIndex => $item) {
																$optionIndex++; ?>
																<span class="lang-<?php echo $optionIndex; ?>"><?php echo $item; ?></span>
															<?php }
															;
															unset($item); ?></label>
													</div>
												...
												foreach ($options as $k => $value): ?>
													<div class="radio"><label><input name="data[Exam][option_selected]"
																value="<?php echo $k; ?>" <?php if ($userExamQuestion['ExamStat']['option_selected'] == $k) {
																	   echo 'checked=checked';
																   } ?>align="" type="radio">
															<?php foreach ($value as $optionIndex => $item) {
																$optionIndex++; ?>
								...
								<?php if ($userExamQuestion['Question']['Qtype']['type'] == "T") {
									...
									<tr>
										<td>
											<div class="radio"><label><input name="data[Exam][true_false]" value="True" <?php if ($userExamQuestion['ExamStat']['true_false'] == "True") {
												echo 'checked=checked';
											} ?> type="radio">
													<?php
											...
											<div class="radio"><label><input name="data[Exam][true_false]" value="False" <?php if ($userExamQuestion['ExamStat']['true_false'] == "False") {
												echo 'checked=checked';
											} ?> type="radio"> <?php
											  foreach ($languageArr as $k => $item) {
												  $k += 1; ?>
					...
					<div class="" style="margin-top: 10px">
						<?php if ($userExamQuestion['Exam']['instant_result'] == 1) { ?>
							...
							<?php } else { ?>
								...
								<?php } ?>
								<a class="btn btn-primary btn-block" onclick="callPrev(<?php echo $quesNo; ?>);"><i
			...
			unset($k, $userExamQuestion, $mainSubjectName, $subTempId, $subjectSection); ?>
				...
				<div id="navbar" class="exam_question_palette ">
					<div class="col-xl-3 col-lg-3 col-md-3 col-sm-12" id="_navbar_exam">
						<div id="timer">
							...
							<div class="exam-PalleteButtons">
								<?php foreach ($userSectionQuestion as $subjectName => $quesArr):
									...
									foreach ($quesArr as $value):
										$quesNo = $value['ExamStat']['ques_no'];
										if ($quesNo == 1)
											$btnType = "btn-default";
										else
											$btnType = ""; ?>
										<div class="col-md-2 col-sm-2 col-xs-2 mrg-1">
											<?php echo $this->Form->button($quesNo, array('type' => 'button', 'onclick' => "navigation($quesNo)", 'id' => "navbtn$quesNo", 'class' => "exam-ButtonNotVisited")); ?>
										</div>
									<?php endforeach;
									unset($quesArr); ?>
								<?php endforeach;
						...
						<div class="exam-divLegend">
							...
							<table>
								<tbody>
									<tr>
										<td>
											<button class="exam-ButtonAnswered"
												onclick="FilterPaletteButtons('ans'); return false;"
												id="countAnswered">0
											</button>
										...
										<td>
											<button class="exam-ButtonNotAnswered"
												onclick="FilterPaletteButtons('notans'); return false;"
												id="countNotAnswered">0
											</button>
									...
									<tr>
										<td colspan="2">
											<button class="exam-ButtonAnsweredMarked"
												onclick="FilterPaletteButtons('ansmarked'); return false;"
												id="countAnsMarked">0
											</button>
							...
							<div class="exam-divQuestionFilter">
								...
								<select id="exam-ddlQStatus">
									...
									<option value="ansmarked"><?php echo __('Answered &amp; marked for review'); ?>
									</option>
						...
						<div class="" style="padding-top: 10px;">
							...
							if ($post['Exam']['exam_mode'] == "T") {
								if ($remainingSection > 0) { ?>
									<div class="col-xs-6">
										<?php echo $this->Form->button(__('Submit Section'), array('type' => 'button', 'id' => 'exam_submit_section', 'class' => "btn btn-danger btn-sm btn-block")); ?>
									</div>
									<?php
								} else {
									$isSubmitButton = true;
					...
					<div class="modal-content">
						<div class="modal-header">
							<h4 class="modal-title"><i
									class="fa fa-exclamation-triangle"></i>&nbsp;<?php echo __('Navigated Away'); ?>
							</h4>
						</div>
						<div class="modal-body">
							...
							<blockquote><?php echo $userValue['Student']['name']; ?>
								,
								<?php echo __('you had navigated away from the test window. This will be reported to Moderator'); ?>
							</blockquote>
							</p>
							<p>
							<blockquote><span
									class="text-danger"><?php echo __('Do not repeat this behaviour'); ?></span>
								<?php echo __('Otherwise you may get disqualified'); ?>
							</blockquote>
							</p>
					...
					<tbody>
						<tr>
							<td align="left" valign="top">
								...
								<p><?php echo str_replace("<script", "", $post['Exam']['instruction']); ?></p>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<div id="QuestionPaperBlock" class="popup exam-overlay" style="display: none;">
			<div class="exam-QuestionPaperHeader">
				<center>
					<h2><?php echo __('Question Paper'); ?></h2>
				</center>
			</div>
			<div id="QuestionPaperData">
													...
													<?php if (strlen($userExamQuestion['Question']['answer']) > 1) {
														foreach ($options as $k => $value): ?>
															<div class="checkbox"><label><input
																		...
																		value="<?php echo $k; ?>" type="checkbox"> <?php foreach ($value as $optionIndex => $item) {
																			   $optionIndex++; ?>
																		<span
				...
				<center>
					<div class="exam-divbackButton">
						<button id="questionPaperBack"
							class="btn btn-primary btn-block"><?php echo __('Back'); ?></button>
					</div>
				</center>
			</div>
		</div>
		...
		<script type="text/javascript">
			$(document).ready(function () {
				...
				<?php foreach ($userExamQuestionArr as $k => $userExamQuestion):
					...
					<?php } elseif ($userExamQuestion['ExamStat']['opened'] == 1 && $userExamQuestion['ExamStat']['answered'] == 0 && $userExamQuestion['ExamStat']['review'] == 0) {
						?>ChangeClassOFPaletteButton('notans', <?php echo $quesNo; ?>);
					<?php } elseif ($userExamQuestion['ExamStat']['answered'] == 0 && $userExamQuestion['ExamStat']['review'] == 1) {
						?>ChangeClassOFPaletteButton('notansmarked', <?php echo $quesNo; ?>);
					<?php } elseif ($userExamQuestion['ExamStat']['answered'] == 1 && $userExamQuestion['ExamStat']['review'] == 1) {
						?>ChangeClassOFPaletteButton('ansmarked', <?php echo $quesNo; ?>);
					<?php }
				endforeach;
				unset($userExamQuestion); ?>
			...
			function exam_submit_section() {
				$.msgBox({
					...
					success: function (result) {
						if (result == "Yes") {
							window.location =
								'<?php echo $this->Html->url(array('controller' => 'Exams', 'action' => 'finish', $post['Exam']['id'], 'Y')); ?>';
						}
					}
				});
			}
