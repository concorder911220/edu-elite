...
<head>
    ...
    if ($mathEditor) echo $this->Html->script('https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.1/MathJax.js?config=TeX-MML-AM_HTMLorMML');
...
<div class="col-md-12">
    <div class="col-md-9"><?php if (strlen($frontLogo) > 0) { ?><?php echo $this->Html->image($frontLogo, array('alt' => $siteName, 'class' => 'global-logo'));
        } else { ?>
            <div class="exam-logo"><?php echo $siteName; ?></div>
        <?php } ?>
    </div>
    <?php if (strtolower($this->params['controller']) == "exams" && strtolower($this->params['action']) == "crm_start") { ?>
...
<div id="exam-footer">
	<div class="copyright">
		<span><strong><?php echo __('Powered by'); ?> </strong><?php echo $this->Html->Link($sitepowerdby, $sitepowerdlink, array('target' => '_blank')); ?></span>
		<div class="pull-left"><?php echo __('Copyright &copy;'); ?> <?php echo $this->Time->format('Y', time()); ?>
			<strong> <?php echo $siteName; ?></strong></div>
