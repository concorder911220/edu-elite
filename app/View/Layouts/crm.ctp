                ...
                <?php if ($this->Session->check('Student')) { ?>
                    <ul class="nav navbar-nav navbar-right" style="color: #000;">
                        <li class="btn-menu-toggle">
                            <div id="menu-toggle" class="show-collapsed hamburger-menu"><i class="fa fa-bars fa-2x"></i></div>
                        </li>
                ...
                <div class="sidebar-collapse sidebar-scroll">
                    <ul id="sidebar-main" class="nav">
                        <?php foreach ($menuArr as $menuName => $menu): $menuIcon = $menu['icon'];
                            h($menuName); ?>
                            <li <?php echo (strtolower($this->params['controller']) == strtolower($menu['controller'])) ? "class=\"active\"" : ""; ?>><?php echo $this->Html->link("<i class=\"icon- $menuIcon\"></i><span class=\"menu-title\">&nbsp;$menuName</span>", array('controller' => $menu['controller'], 'action' => $menu['action']), array('escape' => false)); ?></li>
                        <?php endforeach;
                        unset($menu);
                        unset($menuName);
                        unset($menuIcon); ?>
