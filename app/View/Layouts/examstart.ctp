...
<head>
	...
	if ($mathEditor && $post['Exam']['math_editor_type']=="1") echo $this->Html->script('https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.1/MathJax.js?config=TeX-MML-AM_HTMLorMML');
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="exam_top_header" style="height: 65px;display: none;">
	...
	<div class="col-md-3 col-sm-6 col-xs-4">
		<?php if ($post['Exam']['exam_mode'] == "T") { ?>
			<div class="main-timer1" style="height:63px;background-color: #9E9E9E;"><strong><?php echo __('Total Time');?></strong><div id="maincount1"></div></div>
		<?php }?>
	</div>
	<div class="col-md-offset-1 col-md-3 hidden-xs hidden-sm">
		...
		<button class="btn btn-primary pause_screen" onclick="submitPause();" style="display: none;"><?php echo __('Pause'); ?></button>
	</div>
	<?php if (strtolower($this->params['controller']) == "exams" && strtolower(str_replace($this->params['prefix']."_","",$this->params['action'])) == "start") { ?>
		<div class="col-md-2 col-sm-3 col-xs-2 exam-photo"><?php echo $this->Html->image($studentImage, array('height' => '50', 'title' => $UserArr['Student']['name'])); ?></div><?php } ?>
</div>
...
<div id="scriptCalcUrl" style="display: none;"><?php echo $this->Html->url(array('crm'=>true,'controller'=>'Calculators','action'=>'index'));?></div>
</body>
</html>
